...
<div class="w-[95%] sm:w-[90%] max-w-2xl mx-auto pt-24 md:pt-32 pb-16">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8 mb-6">
        ...
        @if ($post->images && $post->images->isNotEmpty())
            <div class="mb-4 flex flex-col gap-4">
                @foreach ($post->images as $image)
                    <img src="{{ asset($image->image_url) }}" alt="Post image" class="w-full max-h-[500px] object-cover rounded-2xl border border-gray-100">
                @endforeach
            </div>
        @endif
        ...
        @if ($post->gif_url)
            <div class="mb-4">
                <img src="{{ $post->gif_url }}" alt="GIF" class="w-full max-h-[500px] object-cover rounded-2xl border border-gray-100">
            </div>
        @endif
    ...
    <div class="bg-white rounded-3xl border border-gray-100 p-6 mb-6 text-center text-sm text-gray-500">
        <a href="{{ route('showLogin') }}" class="text-[#094174] font-bold hover:underline">Log in</a> to reply to this post.
    </div>
    @endauth
