...
@section('content')
    <div class="pt-32 bg-[#F8F9FA] min-h-screen">
        <form id="explore-form" method="GET" action="{{ route('explore') }}"
            class="w-[90%] mx-auto pb-20 flex flex-col lg:flex-row items-start gap-8">
            <aside
                class="flex flex-col gap-8 pb-4 mb-8 w-full lg:w-64 lg:sticky lg:top-32 lg:max-h-[calc(100vh-9rem)] lg:overflow-y-auto shrink-0">
            ...
            <div class="w-full min-w-0">
                            ...
                            <div class="pt-5 pb-2 px-2 flex flex-col grow">
                                ...
                                <div class="flex flex-wrap items-center gap-3 mb-3">
                                    <div class="flex items-center gap-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-[#094174]"
                                        ...
                                        <span
                                            class="text-xs font-bold text-[#094174]">{{ optional($mountain->province)->name ?? 'Unknown Region' }}</span>
                                    </div>
                                    <div class="flex items-center gap-1">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        <span
                                            class="text-xs font-bold text-[#094174] capitalize">{{ $mountain->difficulty }}</span>
                                    </div>
                                </div>
                ...
                @if ($mountains->hasPages())
                    <div class="explore-pagination mt-14 mb-10 lg:ml-7">
                        {{ $mountains->links('vendor.pagination.tailwind') }}
                    </div>
                @endif
            </div>
        </form>
