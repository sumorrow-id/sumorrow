...
<div class="w-[95%] max-w-3xl mx-auto pt-32 pb-16">
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
            <h1 class="text-2xl font-bold text-[#1a2b4c]">Edit Profile</h1>
        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="p-8">
            ...
            <div class="mb-8">
                ...
                <div class="relative h-48 w-full rounded-2xl overflow-hidden bg-gray-100 group border-2 border-dashed border-gray-200 hover:border-[#094174]/50 transition-colors">
                    ...
                    @endphp
                    <img src="{{ $coverSrc }}" id="cover-preview" alt="Cover preview" class="w-full h-full object-cover">
                    ...
                    <input type="file" id="cover-upload" name="cover" class="hidden" accept="image/*" onchange="previewImage(this, 'cover-preview')">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-8 mb-8">
                ...
                <div class="flex-shrink-0">
                    ...
                    <div class="relative w-32 h-32 rounded-full overflow-hidden bg-gray-100 group border-4 border-white shadow-md">
                        ...
                        @endphp
                        <img src="{{ $avatarSrc }}" id="avatar-preview" alt="Avatar preview" class="w-full h-full object-cover">
                        ...
                        <input type="file" id="avatar-upload" name="avatar" class="hidden" accept="image/*" onchange="previewImage(this, 'avatar-preview')">
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        @if(!$user->hasVerifiedEmail())
                            <p class="mt-2 text-sm text-amber-600 flex items-start gap-1.5">
                                ...
                                <span>Your email is unverified. <button type="button" onclick="document.getElementById('verify-form').submit()" class="font-bold underline hover:text-amber-700">Send verification link</button></span>
                            </p>
                        @endif
                    </div>
                </div>
            ...
            <div class="mb-8">
                ...
                <textarea id="bio" name="bio" rows="4" placeholder="Tell us about your hiking journey..."
                ...
                @error('bio')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
                <p class="mt-1.5 text-xs text-gray-500 text-right" id="bio-counter">0 / 500 characters</p>
            </div>

            <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
                <a href="{{ route('profile') }}" class="px-6 py-2.5 rounded-xl font-medium text-gray-600 hover:bg-gray-100 transition-colors">Cancel</a>
                <button type="submit" class="px-6 py-2.5 rounded-xl font-bold text-white bg-[#094174] hover:bg-[#105DA3] shadow-md shadow-[#094174]/20 transition-all hover:-translate-y-0.5">Save Changes</button>
            </div>
        </form>
