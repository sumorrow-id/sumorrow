...
<!-- Gear Modal -->
<div id="gear-modal" class="fixed inset-0 z-100 flex items-center justify-center hidden" data-has-errors="{{ $errors->any() ? 'true' : 'false' }}">
    <!-- Backdrop -->
    <div class="close-gear-modal-btn absolute inset-0 bg-[#0F172A]/40 backdrop-blur-sm"></div>
    ...
    <div class="relative bg-white rounded-3xl w-full max-w-md p-6 sm:p-8 m-4 shadow-2xl transform transition-all">
        ...
        <form id="gear-form" method="POST" action="{{ route('gears.store') }}" data-store-url="{{ route('gears.store') }}">
            @csrf
