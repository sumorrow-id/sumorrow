...
<header class="mb-10">
    <p class="text-[11px] font-bold text-glacial-teal uppercase tracking-[0.25em]">Overview</p>
    <h1 class="text-4xl font-extrabold text-heading mt-2">Control Center</h1>
    <p class="text-sm text-lithic-blue mt-2">{{ now()->format('l, j F Y') }} &middot; Signed in as <span class="font-bold text-deep-midnight">{{ auth()->user()->username }}</span></p>
</header>
...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            ...
            <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest">Total Users</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            ...
            <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest">New This Week</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            ...
            <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest">Total Mountains</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            ...
            <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest">Active Routes</p>
        </div>
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
    ...
    <section class="xl:col-span-2 bg-white rounded-3xl border border-morning-mist/70 shadow-sm overflow-hidden">
        <div class="px-8 py-6 border-b border-morning-mist/60 flex justify-between items-center">
            <h3 class="text-lg font-bold text-deep-midnight">Recently Registered Users</h3>
            <a href="{{ route('admin.user-updates') }}" class="text-sm font-semibold text-summit-blue hover:underline">View all</a>
        </div>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-morning-mist/20">
                        ...
                        <th class="px-8 py-3.5 text-[10px] font-bold text-lithic-blue uppercase tracking-widest text-right">Joined</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-morning-mist/50">
                    ...
                    <tr class="hover:bg-morning-mist/10 transition-colors">
                        ...
                        <td class="px-8 py-4">
                            @if ($user->role === 'admin')
                                <span class="px-3 py-1 text-[10px] font-bold bg-summit-blue text-white rounded-full uppercase tracking-wider">Admin</span>
                            @else
                                <span class="px-3 py-1 text-[10px] font-bold bg-morning-mist/40 text-blue-bird rounded-full uppercase tracking-wider">User</span>
                            @endif
                        </td>
    ...
    <section class="bg-white rounded-3xl border border-morning-mist/70 shadow-sm p-8 h-fit">
        <h3 class="text-lg font-bold text-deep-midnight mb-5">Quick Actions</h3>
        <div class="space-y-3">
            <a href="{{ route('admin.mountains.create') }}" class="flex items-center gap-4 p-4 rounded-2xl border border-morning-mist/70 hover:border-summit-blue/40 hover:bg-morning-mist/10 transition-colors group">
                ...
                <div>
                    <p class="text-sm font-bold text-deep-midnight group-hover:text-summit-blue">Add a mountain</p>
                    <p class="text-xs text-lithic-blue">Register a new peak in the catalog</p>
                </div>
            </a>
            <a href="{{ route('admin.user-updates') }}" class="flex items-center gap-4 p-4 rounded-2xl border border-morning-mist/70 hover:border-summit-blue/40 hover:bg-morning-mist/10 transition-colors group">
                ...
                <div>
                    <p class="text-sm font-bold text-deep-midnight group-hover:text-summit-blue">Manage users</p>
                    <p class="text-xs text-lithic-blue">Change roles or remove accounts</p>
                </div>
            </a>
            <a href="{{ route('admin.users.export') }}" class="flex items-center gap-4 p-4 rounded-2xl border border-morning-mist/70 hover:border-summit-blue/40 hover:bg-morning-mist/10 transition-colors group">
                ...
                <div>
                    <p class="text-sm font-bold text-deep-midnight group-hover:text-summit-blue">Export users (CSV)</p>
                    <p class="text-xs text-lithic-blue">Download the full member list</p>
                </div>
            </a>
        </div>
    </section>
