...
@section('title', __('admin.user_updates_title') . ' - Sumorrow Admin')
@section('page_title', __('admin.nav_user_updates'))

@section('admin_content')
<header class="mb-10">
    <p class="text-[11px] font-bold text-glacial-teal uppercase tracking-[0.25em]">{{ __('admin.nav_management') }}</p>
    <h1 class="text-4xl font-extrabold text-heading mt-2">{{ __('admin.community_growth') }}</h1>
    <p class="text-sm text-lithic-blue mt-2">{{ __('admin.community_growth_subtitle') }}</p>
</header>

<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest mb-2">{{ __('admin.total_users') }}</p>
        <h3 class="text-3xl font-extrabold text-deep-midnight tabular-nums">{{ number_format($totalUsers) }}</h3>
    </div>
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest mb-2">{{ __('admin.new_this_week') }}</p>
        <h3 class="text-3xl font-extrabold text-deep-midnight tabular-nums">{{ number_format($newThisWeek) }}</h3>
    </div>
    <div class="bg-white p-6 rounded-3xl border border-morning-mist/70 shadow-sm">
        <p class="text-xs font-bold text-lithic-blue uppercase tracking-widest mb-2">{{ __('admin.new_this_month') }}</p>
...
<section class="bg-white rounded-3xl border border-morning-mist/70 shadow-sm overflow-hidden">
    <div class="px-8 py-6 border-b border-morning-mist/60 flex flex-wrap justify-between items-center gap-4">
        <h3 class="text-lg font-bold text-deep-midnight">{{ __('admin.registered_users') }}</h3>
        <div class="flex items-center gap-3">
            <form method="GET" action="{{ route('admin.user-updates') }}" role="search" class="relative">
                ...
                <input type="search" name="q" value="{{ $search }}" placeholder="{{ __('admin.search_user_placeholder') }}"
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-morning-mist/20">
                    ...
                    <th class="px-8 py-3.5 text-[10px] font-bold text-lithic-blue uppercase tracking-widest">{{ __('admin.table_joined') }}</th>
            ...
            <tbody class="divide-y divide-morning-mist/50">
                @forelse ($recentUsers as $user)
                    <tr class="hover:bg-morning-mist/10 transition-colors">
                        ...
                        <td class="px-8 py-4">
                            <span class="text-xs text-lithic-blue font-semibold whitespace-nowrap" title="{{ $user->created_at?->translatedFormat('j F Y H:i') }}">
                                {{ $user->created_at?->diffForHumans() }}
                            </span>
                        </td>
                        <td class="px-8 py-4">
                            @if ($user->id === auth()->id())
                                <span class="px-3 py-1 text-[10px] font-bold bg-summit-blue text-white rounded-full uppercase tracking-wider">{{ __('admin.role_you', ['role' => $user->role]) }}</span>
                            @else
                                <form method="POST" action="{{ route('admin.users.role', $user) }}">
                                    @csrf
                                    @method('PATCH')
                                    <label class="sr-only" for="role-{{ $user->id }}">{{ __('admin.role_for', ['username' => $user->username]) }}</label>
                                    <select id="role-{{ $user->id }}" name="role" onchange="this.form.submit()"
                                        ...
                                        <option value="user" @selected($user->role === 'user')>{{ __('admin.role_user') }}</option>
                                        <option value="admin" @selected($user->role === 'admin')>{{ __('admin.role_admin') }}</option>
                                    </select>
                                </form>
                            @endif
                        </td>
                        <td class="px-8 py-4 text-right">
                            @if ($user->id !== auth()->id())
                                <form method="POST" action="{{ route('admin.users.destroy', $user) }}" class="inline" onsubmit="return confirm({{ Illuminate\Support\Js::from(__('admin.confirm_delete_user', ['username' => $user->username])) }})">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-8 py-14 text-center">
                            @if ($search !== '')
                                <p class="text-sm font-bold text-deep-midnight">{!! __('admin.no_users_match', ['search' => e($search)]) !!}</p>
                                <a href="{{ route('admin.user-updates') }}" class="mt-2 inline-block text-sm font-semibold text-summit-blue hover:underline">{{ __('admin.clear_search') }}</a>
                            @else
