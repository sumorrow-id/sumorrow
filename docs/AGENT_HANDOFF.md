...
## 2026-07-04 — branch: `feature/mycommunity` (merge of `main`)

By: Claude Code

**What changed**

- Resolved the 16-file merge conflict between `feature/mycommunity` and `main`, keeping both sides: mycommunity features (create/join/leave/show community, events, post save/bookmark, community posts) and main's work (localization EN/ID, summit-log handling, admin dashboard revamp, Vite JS modules, `redirect.admin` route group).
- Standardized on the branch's `posts.user_id` rename: converted all remaining `author`/`author_id` references from main-side code (controllers, blades, tests) to `user`/`user_id`.
- Migration fixes: restored the shared `create_posts_table` and `create_post_replies_table` migrations to their original `author_id` form (the branch had edited them in place, which broke `migrate:fresh` because the rename migration then had nothing to rename); added a new guarded migration renaming `post_replies.author_id` → `user_id`; rewrote the corrupted `create_events_table` migration (two class bodies pasted together, and `user_id` now char(36) to match the UUID users PK).
- MyCommunity bug fixes: removed a leftover `dd()` in `EventController::store` that blocked event creation; added the missing `EventController::destroy`; removed a duplicate `destroy()` in `PostController` (PHP fatal); ownership check now compares UUID strings (was cast to int, always failing); `community/show.blade.php` used non-existent columns/routes (`->name` → `->username`, `community->user_id` → `created_by`, `route('login')` → `showLogin`, `elevation` → `elevation_masl`) and now loads the full member list for the Members tab.
- Feed: save/bookmark buttons kept and their JS moved into `resources/js/features/FeedLike.js` (inline scripts removed, matching main's Vite-module convention); forum feeds exclude community posts (`whereNull('community_id')`) and tag-less summit logs on both `/community` and `/community/forum`.
- `category_tags` validation is now `required_without:community_id` so community posts may skip tags while forum posts still require one.

**How to verify**

- `php artisan test --compact` — 253 passing (1333 assertions).
- `php artisan route:list --except-vendor` — 71 routes, includes `community.show`, `community.update-image`, `community.events.*`, `community.posts.save`.
